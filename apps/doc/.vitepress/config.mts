...
const vegaPkg = JSON.parse(
  readFileSync(resolve(__dirname, "../../../packages/vega/package.json"), "utf-8"),
) as { version: string };

const apiSidebarPath = resolve(__dirname, "../api/typedoc-sidebar.json");

function stripMdFromLinks(items: DefaultTheme.SidebarItem[]): DefaultTheme.SidebarItem[] {
...
function loadApiSidebar(): DefaultTheme.SidebarItem[] {
  ...
  const raw = JSON.parse(readFileSync(apiSidebarPath, "utf-8")) as DefaultTheme.SidebarItem[];
...
export default defineConfig({
  ...
  themeConfig: {
    ...
    socialLinks: [{ icon: "github", link: "https://github.com/GyeongHoKim/vega" }],
    footer: {
